...
$user = new select();
$user->pick('user', 'id, email, balance', 'username,secpin', "'$username','$xpassword'", '', 'log', '', '', '=,=', 'and');
if($user->count < 1)
{
	echo 13;
}
else
{
 ...
 $sql="INSERT INTO paylog (user_id,user,product,phone,email,billersCode,variation_code,amount,date,transaction_id,status,rstatus) VALUES('$username','$urow[0]','sendsms','$dest','$email','$billersCode','$variation_code','$cost','$now','$transaction_id','success','$rstatus')";
